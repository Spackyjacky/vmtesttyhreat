#!/usr/bin/env python3
"""
ThreatPad Executable Builder - No Spell Checker Version
Creates standalone executable using PyInstaller without spell checking
"""
...
def build_executable():
    """Build the executable using PyInstaller without spell checker"""
    
    # PyInstaller command options - excluding spell checker
    cmd = [
        "pyinstaller",
        "--onefile",                    # Single executable file
        "--windowed",                   # No console window (GUI app)
        "--name=ThreatPad",            # Executable name
        "--exclude-module=pyspellchecker",  # Exclude spell checker
        "--exclude-module=spellchecker",    # Exclude spell checker
    ...
    # Add icon if exists
    if os.path.exists("threatpad.ico"):
        cmd.insert(-1, "--icon=threatpad.ico")
    else:
        print("Note: No icon file found, building without custom icon")
    
    # Add templates if folder exists
    if os.path.exists("templates"):
        cmd.insert(-1, "--add-data=templates:templates")
    else:
        print("Note: No templates folder found")
    
    print("Building executable (without spell checker)...")
...
def main():
    """Main build process"""
    print("=" * 60)
    print("ThreatPad Executable Builder (No Spell Checker)")
    print("=" * 60)
    ...
    if build_executable():
        ...
        print("4. Spell checking features are disabled in this build")
