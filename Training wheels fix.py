...
def setup_ui(self):
    ...
    self.main_paned.pack(fill="both", expand=True, padx=5, pady=(5, 0))
...
TRAINING_CHECKLISTS = {
    ...
    "Rare and Potentially High-Risk Office Operations": [
        ("TRIAGE", [
            ...
            "Determine whether this is a first-time or rare operation for this user — check the baseline period used by the detection rule",
            "Confirm whether the operation was performed interactively by the user or via an app/service principal",
            "Check whether the operation was performed from a known trusted IP, location, and device",
            "Assess the sensitivity of the operation: does it grant access, move data, or disable controls?",
        ]),
        ("INVESTIGATION", [
            "Pull the full Unified Audit Log (UAL) entry for the flagged operation — note ClientIP, UserAgent, and ResultStatus",
            "Review the user's UAL activity for the past 7 days — look for other anomalous operations",
            "Check sign-in logs for the account: unusual location, IP, device, or time of access",
            "Verify whether MFA was satisfied for the session that performed the operation",
            "Identify what the operation acted on: which mailbox, site, resource, or tenant setting was affected",
            "If mailbox delegation or forwarding was set: identify the recipient and verify legitimacy",
            "If eDiscovery or content search was run: identify the scope and whether results were exported",
            ...
            "Review the user's recent helpdesk tickets or change requests to determine if this was authorised",
        ]),
        ("IOC ENRICHMENT", [
            "Check the source IP against AbuseIPDB and threat intel feeds",
            "Check the UserAgent string for anomalies (automation tools, unusual browsers, scripting frameworks)",
            "If an app or service principal performed the operation: review its permissions and recent activity",
            "Search SIEM for same operation performed by other accounts in the same window",
        ]),
        ("CONTAINMENT", [
            "If the operation is confirmed unauthorised: disable the account and revoke all active sessions and tokens",
            ...
            "If admin role was assigned: remove the role immediately and audit what the new admin account did",
            "Block the source IP at the network/conditional access layer if external and suspicious",
        ]),
        ("ESCALATION CHECK", [
            "Audit log cleared or disabled? → Escalate to L2 immediately — this is a critical indicator of attacker covering tracks",
            "eDiscovery export completed with sensitive data in scope? → Escalate to L2 / DPO — potential data breach",
            "Admin role granted to unexpected account? → Escalate to L2 / CISO",
            "Account confirmed compromised (impossible travel, MFA bypass)? → Escalate to L2 / Identity team",
            "Multiple rare operations performed in the same session? → Escalate to L2 — indicates deliberate attacker activity",
        ]),
        ("CLOSURE", [
            ...
            "Document the specific operation, resource affected, source IP, and timeline",
            "If data exposure occurred: complete formal data breach assessment",
            "Review and tune the detection rule threshold if the operation was legitimate",
        ]),
    ],
    ...
    "Mass Cloud Resource Deletions Time Series Anomaly": [
        ("TRIAGE", [
            ...
            "Establish the timeframe of the deletions and whether they are ongoing",
            "Check whether this is a known maintenance activity, decommission project, or completely unexpected",
            "Identify whether the deletion was performed by a human user, service principal, or automation runbook",
        ]),
        ("INVESTIGATION", [
            "Pull Azure Activity Log entries for all delete operations in the alert window — note Caller, CallerIpAddress, OperationName, ResourceId",
            "Review the calling account's sign-in history: unusual location, IP, device, or time",
            "Check whether the account is a service principal — review its permissions and any recent changes to its credentials or roles",
            "Identify whether any resources were deleted outside normal business hours or from an unusual location",
            "Check whether resource locks were bypassed or removed before deletion",
            "Review whether backups, snapshots, or soft-delete protections exist for the deleted resources",
            "Search for other anomalous Azure management operations from the same account: role assignments, policy changes, network changes",
            "Check if any alerts fired on the account in the past 30 days: suspicious sign-ins, MFA bypass, credential theft",
            "Identify business impact: which services or workloads have been taken offline by the deletions?",
        ]),
        ("IOC ENRICHMENT", [
            "Check the calling IP against AbuseIPDB and Azure known safe IP ranges",
            "If a service principal: check for recent credential rotation, new client secrets, or certificate changes",
            "Search SIEM/Sentinel for other bulk management operations from the same Caller in the past 7 days",
            "Check if the deleted resource IDs match known threat actor techniques (e.g. ransomware destroying cloud backups)",
        ]),
        ("CONTAINMENT", [
            ...
            "Initiate recovery of deleted resources via Azure Recovery Vault, soft-delete, or snapshot restore where available",
            "Notify the resource owners and business stakeholders of the impact",
            "If backups were deleted: assess recovery options and escalate to infrastructure team",
        ]),
        ("ESCALATION CHECK", [
            "Backups or disaster recovery resources deleted? → Escalate to L2 / CISO — potential ransomware or sabotage",
            "Production services offline due to deletions? → Escalate to L2 / CISO and initiate business continuity process",
            "Service principal or admin account confirmed compromised? → Escalate to L2 / Identity team",
            "Deletions appear to be covering tracks after a breach? → Escalate to L2 / IR immediately",
            "Multiple subscriptions or tenants affected? → Escalate to L2 / CISO immediately",
        ]),
        ("CLOSURE", [
            "Confirm the calling account/service principal is secured",
            "Document all deleted resources, their business impact, and recovery status",
            "Confirm resource locks are in place on critical subscriptions",
            "Complete a formal root cause analysis: how did the attacker or automation gain delete permissions?",
            "Recommend Azure RBAC review and principle of least privilege audit",
            "Recommend enabling soft-delete and backup retention policies on all critical resources",
        ]),
    ],

    "Anomalous Remote Access Tool Usage": [
        ("TRIAGE", [
            "Identify the host, user account, and specific remote access tool flagged (e.g. AnyDesk, TeamViewer, ngrok, RustDesk, ScreenConnect, Cobalt Strike beacon)",
            "Determine whether this tool is sanctioned and used legitimately in the environment",
            ...
            "Confirm whether an outbound connection was established and to which external IP/domain",
        ]),
        ("INVESTIGATION", [
            "Pull process creation events for the RAT binary — note full path, hash, parent process, and command-line arguments",
            "Review the tool's network connections: destination IP, port, and protocol",
            "Check the tool's installation directory — legitimate tools install to Program Files; malicious tools often run from temp, AppData, or unusual paths",
            "Submit the file hash to VirusTotal — check for known malicious variants",
            "Check the destination IP/domain against AbuseIPDB and threat intel",
            "Review the host's full process timeline around the alert window for signs of prior compromise",
            "Check for persistence: was the tool added to startup, scheduled tasks, or a service?",
            "Review whether the session resulted in interactive activity: file access, command execution, lateral movement",
            "Search SIEM for the same tool binary hash or C2 IP/domain across other endpoints",
            "If ngrok or similar tunnelling tool: identify what service was being exposed externally",
        ]),
        ("IOC ENRICHMENT", [
            "Submit tool binary hash to VirusTotal",
            "Check destination IPs and domains against AbuseIPDB, VirusTotal, and Shodan",
            "Search for the tool's known C2 infrastructure patterns in threat intel",
            "Check if the tool's installation was preceded by a phishing email, malicious document, or lateral movement event",
        ]),
        ("CONTAINMENT", [
            "Isolate the host from the network if an active RAT session is confirmed or suspected",
            "Terminate the RAT process if still running",
            "Block the destination IP and domain at firewall and proxy",
            "Remove persistence mechanisms (startup entry, scheduled task, service)",
            "Disable the affected user account and revoke active sessions",
            "Quarantine and hash-block the RAT binary",
        ]),
        ("ESCALATION CHECK", [
            "Confirmed active C2 session — attacker has interactive access? → Escalate to L2 / IR immediately",
            "Tool installed by malware dropper or phishing? → Escalate to L2 / IR",
            "Evidence of lateral movement from the compromised host? → Escalate to L2",
            "Multiple hosts running the same RAT? → Escalate to L2 / IR — potential active intrusion",
            "ngrok or tunnel exposing internal services to the internet? → Escalate to L2 immediately",
        ]),
        ("CLOSURE", [
            "Confirm host is isolated, tool removed, and persistence cleared",
            "Confirm all C2 infrastructure is blocked at network level",
            "Document binary hash, C2 IP/domain, persistence mechanism, and full process chain",
            "Confirm no lateral movement occurred or that all affected hosts have been identified",
            "Recommend EDR policy tuning to block or alert on unsanctioned remote access tools",
            "If legitimate tool: implement an approved RAT allowlist and usage policy",
        ]),
    ],

    "Time Series Anomaly for Data Size Transferred to Public Internet": [
        ("TRIAGE", [
            "Identify the source host, user account, and destination (public IP or domain) responsible for the anomalous transfer",
            "Determine the volume flagged and how it compares to the baseline — is this 2x, 10x, or 100x normal?",
            "Identify the protocol and application responsible: HTTPS, FTP, SMB over VPN, cloud sync client?",
            "Establish whether this transfer was during or outside business hours",
            "Check whether this matches any known scheduled backup, update, or legitimate business activity",
        ]),
        ("INVESTIGATION", [
            "Pull firewall/proxy logs for the source host and destination IP/domain — review full session details: bytes transferred, duration, protocol",
            "Identify the process responsible for the outbound traffic on the host",
            "Review the content type or destination: is it a personal cloud storage (Google Drive, Dropbox, OneDrive personal), file sharing site, or unknown IP?",
            "Check DLP alerts for the same timeframe on the same host or user",
            "Review the host's process timeline: was a staging or archiving tool used (7zip, WinRAR, robocopy)?",
            "Check whether large files were recently created on the host before the transfer",
            "Review the user's recent activity: any recent access to sensitive file shares, document repositories, or databases?",
            "Search SIEM for the same destination IP/domain from other hosts",
            "Check if the destination is a known data exfiltration service or paste site",
            "Review email and web activity for the user around the same timeframe for context",
        ]),
        ("IOC ENRICHMENT", [
            "Check the destination IP against AbuseIPDB and threat intel",
            "Check the destination domain against VirusTotal, URLScan.io, and WHOIS for age/registration details",
            "If cloud storage destination: identify the account name associated with the upload if possible",
            "Search SIEM for any other anomalous outbound transfers in the past 30 days from the same host or user",
        ]),
        ("CONTAINMENT", [
            "Block the destination IP and domain at the proxy/firewall if confirmed malicious or unauthorised",
            "Isolate the host if active exfiltration is ongoing or malware is confirmed",
            "Disable the user account if insider threat or account compromise is suspected",
            "Revoke cloud storage OAuth tokens if a personal cloud account was used",
            "Preserve all relevant logs before any rotation occurs",
        ]),
        ("ESCALATION CHECK", [
            "Sensitive, confidential, or regulated data (PII, financial, IP) confirmed in the transfer? → Escalate to L2 / DPO immediately — potential data breach",
            "Transfer appears to be deliberate staging and exfiltration by an insider? → Escalate to L2 / HR / Legal",
            "Malware confirmed responsible for the transfer? → Escalate to L2 / IR",
            "Multiple hosts transferring to the same external destination? → Escalate to L2 / IR",
            "Transfer volume suggests a complete data set (database dump, bulk file export)? → Escalate to L2 / CISO",
        ]),
        ("CLOSURE", [
            "Confirm exfiltration has stopped and destination is blocked",
            "Document the data volume, destination, responsible process, and data classification",
            "Assess whether a formal data breach notification is required under GDPR or other regulations",
            "Confirm account is secured if compromise was involved",
            "Recommend DLP policy review to detect and block this type of transfer earlier",
            "Recommend data classification labels on sensitive file stores to improve future detection",
        ]),
    ],

    "Email Messages Removed After Delivery": [
        ("TRIAGE", [
            "Identify who or what removed the emails: user self-deletion, admin purge, ZAP (Zero-hour Auto Purge), or inbox rule",
            ...
            "If inbox rule or user action: treat as potentially suspicious and investigate further",
        ]),
        ("INVESTIGATION", [
            "Pull the Unified Audit Log for the RemoveMalware, HardDelete, SoftDelete, or MoveToDeletedItems operations on the affected messages",
            "If ZAP triggered the removal: review the ZAP report in the Defender portal for the full threat verdict",
            "If an inbox rule removed the emails: pull the inbox rule details — name, conditions, action, and when it was created",
            "Identify all mailboxes from which the email was removed — was it targeted or campaign-wide?",
            "Check whether any users interacted with the email before removal: links clicked, attachments opened",
            "For any users who interacted with the email: review their endpoint activity in MDE",
            "Review the sender reputation, SPF/DKIM/DMARC alignment, and sending IP",
            "Check Threat Explorer for the full campaign scope — how many similar emails were delivered across the tenant?",
            "If inbox rule deletion: check for other suspicious inbox rules on the same mailbox (forwarding, deletion by keyword)",
            "Check sign-in logs for the mailbox owner: could an attacker have created the rule?",
        ]),
        ("IOC ENRICHMENT", [
            "Submit URLs from the removed email to VirusTotal / URLScan.io",
            "Submit attachment hashes to VirusTotal",
            "Check sender IP and domain against AbuseIPDB and threat intel",
            "Search SIEM for the same sender, subject line hash, or attachment hash across other users",
        ]),
        ("CONTAINMENT", [
            "If any copies of the email remain in mailboxes: purge them via MDO Content Search or Exchange Admin",
            "Block sender address, sending IP, and any malicious URLs or attachment hashes at the email gateway",
            "If users clicked links or opened attachments before removal: isolate affected endpoints",
            "If inbox rule is responsible: delete the rule and check for account compromise",
            "If account compromise suspected: reset password, revoke sessions, and re-enrol MFA",
        ]),
        ("ESCALATION CHECK", [
            "Users confirmed to have clicked phishing links or opened malicious attachments? → Escalate to L2 / IR per endpoint",
            "Inbox rule created by attacker to auto-delete emails (covering tracks)? → Escalate to L2 — active account compromise",
            "Large-scale campaign targeting multiple users? → Escalate to L2",
            "Malware confirmed delivered and executed before ZAP removed the email? → Escalate to L2 / IR",
        ]),
        ("CLOSURE", [
            ...
            "Document campaign scope, threat verdict, removal method, and any user interaction",
            "If inbox rule was the cause: document rule details and confirm account is secured",
            "Review Defender for Office 365 policies to ensure ZAP is enabled on all relevant policies",
        ]),
    ],

    "TI Map Domain Entity to DeviceNetworkEvents": [
        ("TRIAGE", [
            "Identify the device, user, and process responsible for the DNS resolution or network connection to the threat intelligence-matched domain",
            "Note the threat intelligence classification of the domain: C2, malware distribution, phishing, botnet, cryptomining, etc.",
            "Determine whether the connection was blocked or allowed through",
            "Establish when the connection occurred and whether it is ongoing or a one-time event",
            "Check if the device is a server, workstation, or shared system — this affects priority",
        ]),
        ("INVESTIGATION", [
            "Pull DeviceNetworkEvents from MDE Advanced Hunting for the device and domain in the alert window — note RemoteUrl, RemoteIP, RemotePort, InitiatingProcessFileName, InitiatingProcessCommandLine",
            "Identify the initiating process: what application or service resolved or connected to the domain?",
            "Check whether the domain resolved successfully and whether a full TCP/HTTP session was established",
            "Review the initiating process's parent process chain — was it spawned by a suspicious parent?",
            "Check the process binary hash against VirusTotal",
            "Search DeviceNetworkEvents across the full estate for other devices contacting the same domain",
            "Check for any file downloads from the domain (DeviceFileEvents correlated with the same process/timeframe)",
            "Review the device's full process timeline around the alert for signs of prior exploitation or delivery",
            "Check DNS query logs (Sysmon Event 22 or MDE DNS events) for historical lookups to the domain from the device",
            "Assess whether this is a beaconing pattern: regular intervals, consistent byte sizes — indicative of C2",
        ]),
        ("IOC ENRICHMENT", [
            "Check the domain and resolved IP against VirusTotal, AbuseIPDB, and URLScan.io",
            "Check WHOIS for domain registration age and registrar — newly registered domains are higher risk",
            "Check for passive DNS records: does this domain resolve to known malicious infrastructure?",
            "Search for the domain in threat intelligence platforms (MISP, OpenCTI, or your SIEM TI feed) for actor attribution",
            "Check Shodan for the resolved IP: what services are hosted, and is it known infrastructure?",
        ]),
        ("CONTAINMENT", [
            "Block the domain and resolved IP at DNS, proxy, and firewall immediately",
            "If C2 communication is confirmed: isolate the device from the network",
        ...
        ("ESCALATION CHECK", [
            "Confirmed C2 beacon pattern to known threat actor infrastructure? → Escalate to L2 / IR immediately",
            "File download from the domain confirmed? → Escalate to L2 — potential malware delivery",
            "Multiple devices beaconing to the same domain? → Escalate to L2 / IR — potential active intrusion",
            "Domain linked to a known ransomware or APT actor? → Escalate to L2 / CISO immediately",
            "Server or critical infrastructure connecting to the domain? → Escalate to L2",
        ]),
        ("CLOSURE", [
            ...
            "Confirm no lateral movement or data exfiltration occurred from the affected device",
        ]),
    ],

    "Rare Subscription-Level Operations in Azure": [
        ("TRIAGE", [
            "Identify the specific Azure operation flagged: what resource type and action was performed (e.g. policy assignment, role assignment, diagnostic setting change, security centre config, key vault access policy modification)?",
            "Identify the caller account or service principal and confirm whether this is expected behaviour for that identity",
            "Check whether the operation was performed from a known, trusted IP and location",
            "Determine whether a change request or maintenance window exists that authorises this activity",
            "Note the subscription and resource scope of the operation — tenant-wide changes are higher severity",
        ]),
        ("INVESTIGATION", [
            "Pull the full Azure Activity Log entry: Caller, CallerIpAddress, OperationName, ResourceId, SubscriptionId, Level, ResultType, and EventTimestamp",
            "Review the caller account's sign-in history: check for unusual IP, location, device, or time of access",
            "Check whether MFA was satisfied for the session that performed the operation",
            "Review all Azure Activity Log entries for the same caller in the past 7 days — look for other rare or anomalous operations",
            "Identify the blast radius: what does this operation change, expose, or enable?",
            "If a role assignment was made: identify who was granted what role on which scope — check if the new assignee is known and trusted",
            "If a diagnostic setting was modified or deleted: assess whether security logging has been impaired",
            "If a Key Vault access policy was changed: identify what secrets, keys, or certificates may now be accessible",
            "If a policy assignment was changed: assess whether compliance or security controls have been weakened",
            "Search for concurrent unusual operations across the tenant from the same caller",
        ]),
        ("IOC ENRICHMENT", [
            "Check the caller IP against AbuseIPDB — is it a residential IP, VPN, Tor, or known attacker infrastructure?",
            "If a service principal: review its full permission set and check for recent credential changes (new client secret, certificate rotation)",
            "Search SIEM for the same operation type performed by other accounts in the last 30 days — establish a baseline",
            "Check the Azure AD audit log for the caller account: any recent MFA bypass, conditional access failure, or risky sign-in?",
        ]),
        ("CONTAINMENT", [
            ...
            "Reverse the operation where possible: remove the role assignment, restore the policy, revert the diagnostic setting",
            "Apply resource locks on critical subscriptions to prevent further management operations",
            "Notify the subscription owner and platform/cloud team immediately",
            "If Key Vault access was modified: rotate all secrets and keys that may have been accessed",
        ]),
        ("ESCALATION CHECK", [
            "Diagnostic settings deleted or modified — security logging impaired? → Escalate to L2 immediately",
            "Owner or Contributor role granted at subscription scope? → Escalate to L2 / CISO",
            "Key Vault access policy changed — secrets may be exposed? → Escalate to L2 / platform team",
            "Conditional Access or security policies weakened? → Escalate to L2 / CISO",
            "Service principal confirmed compromised? → Escalate to L2 / Identity team",
            "Multiple anomalous operations performed in the same session? → Escalate to L2 — likely active attacker",
        ]),
        ("CLOSURE", [
            "Confirm the operation has been reversed and the account secured",
            "Document the operation type, resource scope, caller IP, and timeline",
            "Confirm Azure security logging is fully restored if it was impaired",
            "Complete an RBAC audit on the affected subscription — are permissions appropriate?",
            "Recommend implementing Azure Privileged Identity Management (PIM) for just-in-time privileged access",
            "Review and alert on Azure Policy and diagnostic setting changes going forward",
        ]),
    ],
    ...
    "Suspicious DNS Tunnel Behaviour Blocked": [
        ("TRIAGE", [
            ...
            "Confirm the traffic was blocked — verify at which enforcement point (DNS firewall, Defender, proxy)",
            "Note the destination domain flagged for DNS tunnelling behaviour",
            "Determine whether this is a known tunnelling tool (iodine, dnscat2, DNSExfiltrator) or anomalous DNS query patterns",
            "Assess whether any data was successfully exfiltrated or C2 traffic established before blocking",
        ]),
        ("INVESTIGATION", [
            "Pull DNS query logs for the device — look for: unusually long subdomain labels, high query frequency to the same domain, TXT/NULL/MX record queries with encoded data, base32/base64 encoded subdomains",
            "Identify the initiating process responsible for the DNS queries",
            "Submit the process binary hash to VirusTotal",
            "Review the process parent chain — was it spawned by a malicious document, script, or lateral movement?",
            "Check the device's full process and network event timeline for signs of prior compromise",
            "Assess query volume and pattern: is there a beaconing interval suggesting C2 over DNS?",
            "Check whether any data was encoded in the DNS queries — look at subdomain labels for base32/hex patterns",
            "Search across all devices in the SIEM for queries to the same domain",
            "If tunnelling tool is identified: check for persistence mechanisms on the device",
            "Review whether the same domain was queried historically before the block — how long has this been occurring?",
        ]),
        ("IOC ENRICHMENT", [
            "Check the destination domain against VirusTotal, AbuseIPDB, and passive DNS",
            "Check WHOIS for domain registration — newly registered or bulletproof hosting is high risk",
            "Identify the DNS server used — is the device using a custom or external DNS resolver to bypass filtering?",
            "Search threat intel for the domain — is it associated with a known threat actor or malware family?",
        ]),
        ("CONTAINMENT", [
            "Confirm the domain is blocked at all DNS enforcement points across the estate",
            "Isolate the affected device — DNS tunnelling with active C2 is a critical indicator of compromise",
        ...
        ("ESCALATION CHECK", [
            ...
            "Multiple devices using DNS tunnelling to same domain? → Escalate to L2 / IR — potential active intrusion",
            "Sensitive data encoded in DNS queries? → Escalate to L2 / DPO — potential data breach",
        ]),
        ("CLOSURE", [
            "Confirm device is isolated and fully remediated or reimaged",
            "Confirm domain is blocked at all DNS enforcement points",
            "Document the tunnelling tool, domain, query patterns, process chain, and data encoded in queries if identified",
            "Assess whether any data was successfully exfiltrated and initiate breach assessment if required",
            "Recommend DNS logging and anomaly detection improvements (e.g. Sysmon DNS, PDNS, or DNS over HTTPS monitoring)",
...
MDE_CHECKLISTS = {
    "MDI: Possible Golden Ticket Attack": [
        ("TRIAGE", [
            "Identify the affected account and source IP from the alert",
            "Note the ticket type flagged: TGT, TGS, or suspicious PAC",
            "Check if the KRBTGT account has been recently accessed or reset",
            "Determine if this is linked to a CVE-2021-42287 exploit attempt",
        ]),
        ("INVESTIGATION", [
            "Pull all Kerberos authentication events for the affected account in the alert window",
            "Check for forged TGT or TGS tickets with anomalous lifetimes or PAC data",
            "Review domain controller event logs: 4768, 4769, 4771",
            "Search for lateral movement from the source IP around the same timeframe",
            "Check SIEM for prior AS-REP roasting or Kerberoasting alerts on same host",
        ]),
        ("CONTAINMENT", [
            "Reset the KRBTGT account password TWICE (required to invalidate all existing tickets)",
            "Isolate the source host from the network",
            "Revoke all active sessions for the affected account",
            "Block the source IP at firewall/network layer",
        ]),
        ("ESCALATION CHECK", [
            "KRBTGT hash confirmed compromised? → Escalate to L2 / IR immediately",
            "Evidence of domain-wide lateral movement? → Escalate to L2",
            "Multiple domain controllers affected? → Escalate to L2 / CISO",
        ]),
        ("CLOSURE", [
            "Confirm KRBTGT password was reset twice and all tickets invalidated",
            "Document the forged ticket details and source",
            "Verify no persistence mechanisms were left by the attacker",
            "Record full timeline and IOCs in the ticket",
        ]),
    ],
    "MDI: Possible Kerberoasting Attack": [
        ("TRIAGE", [
            "Identify source IP and account making the suspicious TGS-REQ requests",
            "Note which service accounts (SPNs) were targeted",
            "Check whether this was preceded by LDAP reconnaissance alerts",
            "Assess severity: how many SPNs were requested?",
        ]),
        ("INVESTIGATION", [
            "Pull event 4769 (Kerberos service ticket request) from domain controllers",
            "Check for RC4 encryption being requested for service tickets",
            "Review LDAP query logs for SPN enumeration from the same source",
            "Identify all service accounts targeted — check for privileged accounts",
            "Search SIEM for any successful authentications from those service accounts post-alert",
        ]),
        ("CONTAINMENT", [
            "Block the source IP at network layer",
            "Force password resets on targeted service accounts, especially privileged ones",
            "Set service account passwords to 25+ character complex strings",
            "Disable RC4 encryption on sensitive service accounts if possible",
        ]),
        ("ESCALATION CHECK", [
            "Privileged service accounts (SQL, Exchange, admin) targeted? → Escalate to L2",
            "Evidence of successful authentication with cracked credentials? → Escalate to L2 / IR",
            "Part of a broader attack chain? → Escalate to L2",
        ]),
        ("CLOSURE", [
            "Confirm all targeted service account passwords are reset",
            "Verify no lateral movement occurred using cracked credentials",
            "Document targeted SPNs and source IP in ticket",
            "Recommend implementing AES-only Kerberos on sensitive accounts",
        ]),
    ],
    "MDE: Suspicious PowerShell Activity Detected": [
        ("TRIAGE", [
            "Identify the affected host, user account, and process that triggered the alert",
            "Review the flagged PowerShell command or script in the alert details",
            "Check the detection source: AMSI, script block logging, or behaviour-based",
            "Determine whether this is a scheduled task, interactive session, or remote execution",
        ]),
        ("INVESTIGATION", [
            "Pull full PowerShell script block logs (Event 4104) from the affected host",
            "Review parent process: what launched PowerShell?",
            "Check for encoded commands (-EncodedCommand), download cradles (IEX, Invoke-Expression)",
            "Search SIEM for the same script hash or command pattern across other endpoints",
            "Check for network connections made by the PowerShell process",
        ]),
        ("IOC EXTRACTION", [
            "Extract any URLs, IPs, or domains referenced in the script",
            "Submit any dropped files or hashes to VirusTotal",
            "Defang and log all extracted IOCs in the ticket",
        ]),
        ("CONTAINMENT", [
            "Isolate the host if malicious intent is confirmed",
            "Terminate the malicious PowerShell process if still running",
            "Block any C2 IPs or domains identified at the firewall/proxy",
            "Disable the triggering account if compromised",
        ]),
        ("ESCALATION CHECK", [
            "Script contains download cradle or executes remote payload? → Escalate to L2",
            "Evidence of persistence (scheduled task, registry run key)? → Escalate to L2",
            "Lateral movement observed? → Escalate to L2 / IR",
        ]),
        ("CLOSURE", [
            "Confirm host is remediated or reimaged",
            "Confirm IOCs are blocked at network level",
            "Document full script content, parent process, and IOCs in ticket",
        ]),
    ],
    "MDE: Fileless Attack Technique Detected": [
        ("TRIAGE", [
            "Identify affected host, user, and process from the alert",
            "Note the fileless technique flagged: shellcode, process hollowing, reflective injection",
            "Check whether the attack was prevented, blocked, or only detected",
            "Determine the entry point: Office macro, browser exploit, phishing link?",
        ]),
        ("INVESTIGATION", [
            "Pull process memory details and parent/child process chain from MDE alert",
            "Review network connections made by the injected process",
            "Check for privilege escalation attempts by the process",
            "Search SIEM and MDE for same behaviour across other endpoints",
        ]),
        ("CONTAINMENT", [
            "Isolate the affected host immediately",
            "Block identified C2 IPs and domains at firewall/proxy",
            "Terminate the malicious process if still active",
            "Disable any accounts that were running the injected process",
        ]),
        ("ESCALATION CHECK", [
            "Shellcode or process hollowing confirmed? → Escalate to L2 / IR immediately",
            "C2 communication established? → Escalate to L2 / IR",
            "Multiple hosts affected? → Escalate to L2",
        ]),
        ("CLOSURE", [
            "Confirm host has been isolated and forensic image taken if required",
            "Block all identified C2 infrastructure",
            "Document technique, process chain, and network IOCs in ticket",
        ]),
    ],
    "MDO: Email Reported by User as Malware or Phish": [
        ("TRIAGE", [
            "Identify the reporting user and the suspicious email from the alert",
            "Pull the email details: sender, subject, received time, delivery action",
            "Check whether the email was delivered, quarantined, or junked",
            "Determine if the user clicked any links or opened attachments",
        ]),
        ("INVESTIGATION", [
            "Review the email headers: check SPF, DKIM, DMARC alignment",
            "Extract and defang all URLs from the email body and headers",
            "Extract attachment names, types, and hashes if present",
            "Submit URLs to VirusTotal / URLScan.io and hashes to VirusTotal",
            "Check whether other users received the same or similar email",
            "Review MDO AIR investigation results for automated findings",
        ]),
        ("CONTAINMENT", [
            "Soft-delete or hard-delete the email from all affected mailboxes via MDO",
            "Block the sender address and sending IP at the email gateway",
            "Block identified malicious URLs at the proxy/Safe Links policy",
            "If user clicked a link and entered credentials: initiate password reset immediately",
            "If attachment was opened: isolate the endpoint and escalate",
        ]),
        ("ESCALATION CHECK", [
            "User entered credentials on a phishing site? → Escalate to L2 / Identity team",
            "Attachment confirmed malicious and opened? → Escalate to L2 / IR",
            "Multiple users affected across the organisation? → Escalate to L2",
        ]),
        ("CLOSURE", [
            "Confirm email purged from all mailboxes",
            "Confirm all IOCs blocked",
            "Document sender, URLs, hashes, and affected users in ticket",
        ]),
    ],
    "MDC: Successful Brute Force Attack": [
        ("TRIAGE", [
            "Identify the targeted VM/host, account, and service (RDP, SSH, WinRM)",
            "Confirm which authentication attempt succeeded",
            "Note the source IP of the successful login",
        ]),
        ("INVESTIGATION", [
            "Pull authentication logs for the attack window (4624/4625 or auth.log)",
            "Check what actions were performed after the successful login",
            "Review source IP reputation via AbuseIPDB",
            "Check for files dropped, tools installed, or accounts created post-compromise",
            "Search SIEM for lateral movement from the compromised VM",
        ]),
        ("CONTAINMENT", [
            "Block the source IP at NSG/firewall immediately",
            "Reset the compromised account credentials",
            "Revoke all active sessions for the account",
            "Isolate the VM if post-compromise activity is confirmed",
            "Enforce MFA on the targeted service if not already in place",
        ]),
        ("ESCALATION CHECK", [
            "Confirmed post-compromise activity on the VM? → Escalate to L2",
            "Lateral movement from the VM? → Escalate to L2 / IR",
            "Multiple VMs targeted in same campaign? → Escalate to L2",
        ]),
        ("CLOSURE", [
            "Confirm source IP blocked, account secured, sessions revoked",
            "Document attack timeline, source IP, and post-compromise actions",
            "Recommend RDP/SSH exposure review — should not be publicly exposed",
        ]),
    ],
    "MDC: Communication with Suspicious Domain (Threat Intel)": [
        ("TRIAGE", [
            "Identify the host and process making the suspicious connection",
            "Note the domain flagged and the threat intelligence category",
            "Determine whether the connection was blocked or allowed",
        ]),
        ("INVESTIGATION", [
            "Check the domain against VirusTotal, URLScan, WHOIS, and AbuseIPDB",
            "Review the process making the connection and its parent chain",
            "Pull network logs for the full communication session",
            "Search SIEM for other hosts communicating with the same domain",
            "Check for C2 beacon patterns: regular intervals, small payloads",
        ]),
        ("CONTAINMENT", [
            "Block the domain at DNS/proxy/firewall",
            "Isolate the host if active C2 is suspected",
            "Disable any accounts associated with the connecting process",
        ]),
        ("ESCALATION CHECK", [
            "Confirmed C2 communication? → Escalate to L2 / IR",
            "Multiple hosts connecting to same domain? → Escalate to L2",
            "Domain linked to known threat actor? → Escalate to L2",
        ]),
        ("CLOSURE", [
            "Confirm domain blocked across estate",
            "Document process name, domain, and connection details",
            "Update threat intel blocklist with new IOC",
        ]),
    ],
}
...
def setup_training_wheels_panel(self):
    ...
    self.tw_frame = tk.Frame(self.main_paned, relief=tk.RIDGE, bd=1)
    ...
    tw_canvas = tk.Canvas(list_container, highlightthickness=0)
    ...
    def _on_scroll_frame_configure(e):
        tw_canvas.configure(scrollregion=tw_canvas.bbox("all"))
    def _on_canvas_configure(e):
        tw_canvas.itemconfig(self._tw_canvas_window, width=e.width)

    self.tw_scroll_frame.bind("<Configure>", _on_scroll_frame_configure)
    self._tw_canvas_window = tw_canvas.create_window(
        (0, 0), window=self.tw_scroll_frame, anchor="nw"
    )
    tw_canvas.bind("<Configure>", _on_canvas_configure)
    ...
    if self.config.TRAINING_WHEELS:
        self.main_paned.insert(0, self.tw_frame, weight=0)
        self.root.after(100, self._tw_set_sash)

# ── Searchable dropdown helpers ─────────────────────────────────────

def _tw_get_all_types(self):
    return (list(self.TRAINING_CHECKLISTS.keys()) +
            list(getattr(self, 'MDE_CHECKLISTS', {}).keys()))

def _tw_show_popup(self, event=None):
    self._tw_hide_popup()
    query = self.tw_incident_var.get().strip().lower()
    all_types = self._tw_get_all_types()
    ...
    listbox = tk.Listbox(
        ...
        width=52, height=min(14, len(matches)),
    ...
    def on_select(e):
        ...
        if sel:
            ...
            self.tw_incident_var.set(chosen)
            self._tw_hide_popup()
            self._tw_load_checklist()
    ...
    if self._tw_popup and self._tw_popup.winfo_exists():
        try:
            if self._tw_popup_listbox.size() == 1:
                chosen = self._tw_popup_listbox.get(0)
                self.tw_incident_var.set(chosen)
                self._tw_hide_popup()
                self._tw_load_checklist()
                return
...
def _tw_set_sash(self):
    try:
        self.main_paned.sashpos(0, 280 if self.tw_form_mode else 210)
    except Exception:
        pass
...
def _tw_load_checklist(self, event=None):
    ...
    sections = (self.TRAINING_CHECKLISTS.get(incident_type) or
                getattr(self, 'MDE_CHECKLISTS', {}).get(incident_type, []))
    if not sections:
        return
...
def _tw_toggle_mode(self):
    ...
    self.root.after(50, self._tw_set_sash)
    self._tw_load_checklist()

def _tw_build_note(self):
    """Build a clean incident note — answers only, grouped by section."""
    incident_type = self.tw_incident_var.get()
    sections = (self.TRAINING_CHECKLISTS.get(incident_type) or
                getattr(self, 'MDE_CHECKLISTS', {}).get(incident_type, []))
...
def toggle_training_wheels(self):
    ...
    if self.config.TRAINING_WHEELS:
        try:
            self.main_paned.forget(self.tw_frame)
        except Exception:
            pass
        self.main_paned.insert(0, self.tw_frame, weight=0)
        self.root.after(100, self._tw_set_sash)
