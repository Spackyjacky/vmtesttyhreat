...
def setup_ui(self):
    self.setup_menu()
    # PanedWindow fills the space between menu and buttons/status bar.
    # In form mode both panes (TW panel + notebook) are visible with a sash.
...
def setup_training_wheels_panel(self):
    ...
    # tw_frame is a child of main_paned so it can be inserted/removed as a pane.
    ...
    # Add TW pane if enabled in saved settings (notebook not yet created here,
    # so we set the sash position after a short delay).
...
def _tw_set_sash(self):
    """Set sash position after layout has settled."""
    try:
        height = 280 if self.tw_form_mode else 210
        self.main_paned.sashpos(0, height)
    except Exception:
        pass
...
def show_settings_window(self):
    ...
    # Pack button frame FIRST so it is always visible at the bottom
