...
COPY_CLEAR = False   # clear tab after safe copy
COPY_COUNT_WARN = True  # warn if same tab copied more than once
...
```
    ...
    self.tabs = {}
    self.config = Config()
    self._tab_copy_counts = {}  # frame -> int, resets when tab content changes
...
def clear_text(self):
    """Clear only the typed text — preserves widget state (tags, undo history)."""
    current = self.notebook.select()
    if not current:
        return
    current_frame = self.notebook.nametowidget(current)
    ...
    if not text_widget:
        return
    # Only delete the actual content — don't reset tags or widget config
    text_widget.delete("1.0", "end")
    # Reset copy count for this tab since content has changed
...
def safe_copy_text(self):
    ...
    def _do_copy(txt):
        """Perform the actual copy, increment counter, optionally clear."""
        self.root.clipboard_clear()
        self.root.clipboard_append(txt)
        # Increment copy count for this tab
        self._tab_copy_counts[current_frame] = self._tab_copy_counts.get(current_frame, 0) + 1
        count = self._tab_copy_counts[current_frame]
        # Copy-clear: wipe the tab after copying
        if self.config.COPY_CLEAR:
            text_widget.delete("1.0", "end")
            self._tab_copy_counts[current_frame] = 0  # cleared = fresh start
            self.validation_label.config(text="Copied & Cleared", fg="green")
            self.update_status(f"Copied safely — note cleared (copy #{count})")
        else:
            self.validation_label.config(text=f"Copied ({count}x)", fg="green")
            self.update_status(f"Text copied safely (copy #{count} for this tab)")
...
def on_text_change_event(self, event=None):
    ...
    if event and event.widget:
        # Reset copy count when user edits — it's a new/modified note
        current = self.notebook.select()
        if current:
...
def create_general_tab(self, parent):
    ...
    ttk.Checkbutton(
        ...
        variable=self.copy_clear_var
    ).pack(anchor="w", padx=5, pady=2)
    ttk.Label(copy_frame,
              text="Clears the tab content immediately after a successful copy.",
    ...
    ttk.Checkbutton(
        ...
        variable=self.copy_count_warn_var
    ).pack(anchor="w", padx=5, pady=2)
    ttk.Label(copy_frame,
              text="Prompts a warning if Safe Copy is pressed again before the note changes.",
